...
// Simple kernel for computing the saxpy operation with real single precision float datatype 

const char *KernelSource = "\n"						\
  "__kernel void saxpy(           // Scalar  kernel                       \n" \
...
// Simple kernel for computing the saxpy operation with real single precision vector float4 datatype 

const char *KernelSource4 = "\n"					\
  "__kernel void saxpy(           // Vector4 kernel                       \n" \
  "   const unsigned int n,                                               \n" \
  "   const float a,                                                      \n" \
  "   __global float4* x,                                                 \n" \
  "   __global float4* y)                                                 \n" \
...
int main(int argc, char** argv)
{
  ...
  Kernels[0] = NULL;
  Kernels[1] = (char *)KernelSource;
  Kernels[2] = NULL;
  Kernels[3] = NULL;
  Kernels[4] = (char *)KernelSource4;
  ...
  int i = 0;
  unsigned int count = DATA_SIZE, countn;
  ...
  if (gpu == 0) {     // No accelerator or gpu, just cpu
    ...
    pref_vw   = device_reports[a_cpu].vec_width_float;
    printf("There is a cpu, using it with float%d kernel\n", pref_vw);
  }
    ...
    if (a_gpu != -1) { // There is a gpu in our platform
      ...
      pref_vw   = device_reports[a_gpu].vec_width_float;
      printf("Found a gpu, using it with float%d kernel\n", pref_vw);
    } else if (an_accelerator != -1) {
      device_id = devices[an_accelerator];
      pref_vw   = device_reports[an_accelerator].vec_width_float;
      printf("No gpu but found an accelerator, using it with float%d kernel\n", pref_vw);
    } else {
      printf("No cpu, no gpu, nor an accelerator, have to quit\n");
      return EXIT_FAILURE;
    }
  }
  ...
  if(argc == 3) {
    ...
    if (pref_vw == 1 || pref_vw == 4 ) {
      printf(SEPARATOR);	
      printf("\nUser wants to use float%d kernel, overwriting automatically selected!\n\n", pref_vw);
      printf(SEPARATOR);		
    } else {
      printf(SEPARATOR);
      printf("User want's an illegal vector width %d, switching to scalar kernel\n", pref_vw);
      printf(SEPARATOR);
      pref_vw = 1;	
    }
  }
  ...
  program = clCreateProgramWithSource(context, 1, (const char **) &Kernels[pref_vw], NULL, &err);
  ...
  //
  // kernel_name is a function name in the program declared with the __kernel qualifier (here "square").
  // 
  // errcode_ret will return an appropriate error code. If errcode_ret is NULL, no error code is returned.
  //
  ...
  // Create buffer objects for the input and output arrays in device memory for our calculation
  //
  // A buffer object is created using the following function
  //
  // cl_memclCreateBuffer (cl_context context, cl_mem_flags flags,
  ...
  // sizeof(unsigned int) for count argument.
  
  countn = count/pref_vw;         // Data contains countn vectors of selected width

  printf("\n");
  printf(SEPARATOR);
  printf("\nSetting the kernel arguments, computing for %d float%d vectors\n", countn, pref_vw);
  
  err  = 0;
  err  = clSetKernelArg(kernel, 0, sizeof(unsigned int), &countn);
