...
void call_kernel_sph(float *g1_in,float *g2_in,float *x1_in,float *x2_in,float *SmoothLength,float bsz,int nc,int np,float *sdens_out,char * cl_name) {
//----------------------------------------------------------------------------
// Initialization
    size_t global;                      // global domain size for our calculation
    size_t local;                       // local domain size for our calculation
...
// Claim Variables for Device
    ...
    cl_mem input4;                      // device memory used for the input array
    cl_mem input5;                      // device memory used for the input array
    cl_mem output;                      // device memory used for the output array
//----------------------------------------------------------------------------
// Setup Context of OpenCL
	int err;
...
// Allocate Memory for Device
    ...
    input4 = clCreateBuffer(context, CL_MEM_READ_ONLY,  sizeof(float)*np, NULL, NULL);
    input5 = clCreateBuffer(context, CL_MEM_READ_ONLY,  sizeof(float)*np, NULL, NULL);
    output = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float)*nc*nc, NULL, NULL);
//----------------------------------------------------------------------------
// Copy Data to Device
    clEnqueueWriteBuffer(commands, input1, CL_TRUE, 0, sizeof(float)*nc*nc, g1_in, 0, NULL, NULL);
	printf("--------------------------%d\n",err);
    clEnqueueWriteBuffer(commands, input2, CL_TRUE, 0, sizeof(float)*nc*nc, g2_in, 0, NULL, NULL);
	printf("--------------------------%d\n",err);
    clEnqueueWriteBuffer(commands, input3, CL_TRUE, 0, sizeof(float)*np, x1_in, 0, NULL, NULL);
	printf("--------------------------%d\n",err);
    clEnqueueWriteBuffer(commands, input4, CL_TRUE, 0, sizeof(float)*np, x2_in, 0, NULL, NULL);
	printf("--------------------------%d\n",err);
    clEnqueueWriteBuffer(commands, input5, CL_TRUE, 0, sizeof(float)*np, SmoothLength, 0, NULL, NULL);
	printf("--------------------------%d\n",err);
//----------------------------------------------------------------------------
// Passing Parameters into Kernel Functions
	...
	printf("--------------------------%d\n",err);
    err = clSetKernelArg(kernel, 3, sizeof(cl_mem), &input4);
	printf("--------------------------%d\n",err);
    err = clSetKernelArg(kernel, 4, sizeof(cl_mem), &input5);
	printf("--------------------------%d\n",err);
    err = clSetKernelArg(kernel, 5, sizeof(cl_mem), &output);
	printf("--------------------------%d\n",err);
    err = clSetKernelArg(kernel, 6, sizeof(float), &bsz);
	printf("--------------------------%d\n",err);
    err = clSetKernelArg(kernel, 7, sizeof(int), &nc);
	printf("--------------------------%d\n",err);
    err = clSetKernelArg(kernel, 8, sizeof(int), &np);
	printf("passing--------------------------%d\n",err);
//----------------------------------------------------------------------------
// Runing Kernel Functions
    //clGetKernelWorkGroupInfo(kernel, devices[1], CL_KERNEL_WORK_GROUP_SIZE, sizeof(local), &local, NULL);
...
// Free the Memory in Device
    ...
    clReleaseMemObject(input4);
    clReleaseMemObject(input5);
    clReleaseMemObject(output);
//----------------------------------------------------------------------------
    clReleaseProgram(program);
    clReleaseKernel(kernel);
...
int main(int argc, const char *argv[])
{
    ...
    //float *g1_in = (float *)malloc(nc*nc*sizeof(float));
    //float *g2_in = (float *)malloc(nc*nc*sizeof(float));
    ...
    //for (i = 0; i<nc; i++) for (j = 0; j<nc; j++) {
	//	index = i*nc+j;
	//	g1_in[index] = i*dsx+0.5*dsx-bsz/2.0;
...
//--------------------------------------------------------------------
	//for(i=0;i<np;i++) {
  	...
  	//}
	//free(particle);
	//call_kernel_sph(g1_in,g2_in,x1_in,x2_in,SmoothLength,bsz,nc,np,sdens_out,"./dumm_sph_opencl.cl");
	//call_kernel_sph(x1_in,x2_in,x3_in,bsz,nc,np,sdens_out,"./sph_opencl.cl");
	call_kernel_mem(nc,np,sdens_out,"./dumm_sph_opencl.cl");
